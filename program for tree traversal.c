...
num); void preorder(struct rec *tree);
void inorder(struct rec
*tree); void
postorder(struct rec *tree);
int count=1;
main()
{
int
...
again"); getch(); }
}while((selection<1)||(selectio
n>5)); return (selection);
}
...
e
tree=(struct rec
*)malloc(sizeof(struct rec)); tree-
>left=tree->right=NULL;
