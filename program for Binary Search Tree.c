...
struct rec *insert(struct rec *tree,long
num); struct rec *copy(struct rec *tree);
...
; switch(choice)
{
case 1:puts("Enter integers:To quit
enter 0"); scanf("%ld",&digit);
while(digit!=0)
{
...
puts("Enter 2: Copy a tree to another
BST"); puts("Enter 3:
Display(inorder)the BST"); puts("Enter
4: END");
